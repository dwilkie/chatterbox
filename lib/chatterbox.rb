...
module Chatterbox
  ...
  #
  # <tt>blk</tt> is an optional block which can be passed in order to override the publisher that
  # gets used.
  # notify(:summary => "my summary") do |via|
  #   via["Twitter"] => {:twitter_account => "someone@example.com"}
  # end
  def notify(message, &blk)
    if blk
      publisher_options = {}
      blk.call(publisher_options)
      publisher_options.keys.each do |key|
        publisher_options[key.to_s.camelcase] = publisher_options.delete(key)
      end
      message.update(publisher_options)
      message.update({:config => {:skip_publishers => true}})
    end
