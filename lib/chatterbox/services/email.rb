...
module Chatterbox::Services
  class Email
    ...
    def self.deliver(options = {})
      new(options).deliver unless options[:config] && options[:config][:skip_publishers] && !options[self.to_s]
    end
    ...
    def merge_configs
      @options[:config] = @options[self.class.to_s] if @options[self.class.to_s]
      @options[:config] ||= {}
